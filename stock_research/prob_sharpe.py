...
plot_dir = "./industrials_hist/prob_sharpe_plots"
os.makedirs(data_dir, exist_ok=True)
os.makedirs(plot_dir, exist_ok=True)

#------------------------------------------------------------------------------
# List of S&P 500 Industrial tickers and extra sector ticker (using XLI as proxy)
tickers = [
...
# Extra ticker representing industrial sector performance (for comparison) note: using SPY until figure out how to pull sector data
sector_ticker = "SPY"
...
def compute_sharpe_se(overall_sr, skew, kurt, T):
    return np.sqrt((1 - overall_sr * skew + 0.25 * overall_sr**2 * (kurt - 1)) / T)
...
def process_ticker(ticker):
    ...
    lower_bound = overall_sr - 1.96 * sigma_sr
...
# Process the extra (sector) ticker for comparison.
print(f"Processing sector ticker {sector_ticker} ...")
sector_result = process_ticker(sector_ticker)
if sector_result is not None:
    ticker_results[sector_ticker] = sector_result

#------------------------------------------------------------------------------
# Build a summary DataFrame of metrics (exclude the sector ticker for ranking top 5)
summary = {
    "ticker": [],
    "overall_sr": [],
    "sigma_sr": [],
...
for t, metrics in ticker_results.items():
    if t == sector_ticker:
        continue  # skip sector for ranking top 5 stocks
...
# 1. Plot time series (price history) for top 5 stocks and the sector ticker.
plt.figure(figsize=(12, 6))
for ticker in top5_tickers:
    df = ticker_results[ticker]["df"]
    plt.plot(df.index, df["close"], label=ticker)
# Add the sector ticker
if sector_ticker in ticker_results:
    df_sector = ticker_results[sector_ticker]["df"]
    plt.plot(df_sector.index, df_sector["close"], label=f"{sector_ticker}", linestyle="--", linewidth=2)
plt.title("Top 5 Stocks vs Sector - Price Time Series")
plt.xlabel("Date")
plt.ylabel("Close Price")
plt.legend()
plt.grid(True)
plt.savefig(os.path.join(plot_dir, "top5_sector_time_series.png"))
plt.close()

# 2. Plot overall Sharpe ratios with 95% CI error bars for the top 5 stocks plus the sector.
plt.figure(figsize=(10, 6))
tickers_to_plot = top5_tickers + [sector_ticker]
bars = []
errors = []
...
plt.title("Overall Sharpe Ratio with 95% CI (Top 5 Stocks and Sector)")
plt.xlabel("Ticker")
plt.ylabel("Overall Sharpe Ratio")
plt.grid(True)
plt.savefig(os.path.join(plot_dir, "top5_sector_sharpe_confidence.png"))
plt.close()

# 3. Plot daily return distributions for each top ticker and the sector (separate plots).
for ticker in top5_tickers + [sector_ticker]:
...
# 4. Plot quarterly Sharpe ratio distributions for each top ticker and the sector.
for ticker in top5_tickers + [sector_ticker]:
    plt.figure(figsize=(8, 5))
...
# 5. Combined plot: Histogram of quarterly Sharpe ratios for top 5 stocks plus sector.
plt.figure(figsize=(10, 6))
for ticker in top5_tickers + [sector_ticker]:
    qs = ticker_results[ticker]["quarterly_sharpe"]
    plt.hist(qs, bins=30, alpha=0.5, label=ticker)
plt.title("Combined Quarterly Sharpe Ratio Distribution (Top 5 Stocks + Sector)")
plt.xlabel("Quarterly Sharpe Ratio")
plt.ylabel("Frequency")
plt.legend()
plt.grid(True)
